import { Link, useNavigate, useRouterState } from "@tanstack/react-router";
import { useSyncExternalStore } from "react";
import { useTranslation } from "react-i18next";
import {
  LayoutDashboard,
  Users,
  BadgePercent,
  ScanLine,
  Receipt,
  UserPlus,
  CalendarDays,
  Dumbbell,
  BarChart3,
  Settings,
  LogOut,
  UserCog,
} from "lucide-react";
import { cn } from "@/lib/utils";
import { getSession, isOwner, logout, subscribeSession } from "@/lib/auth";

const items = [
  { to: "/", icon: LayoutDashboard, key: "dashboard" },
  { to: "/users", icon: UserCog, key: "users", ownerOnly: true },
  { to: "/members", icon: Users, key: "members" },
  { to: "/plans", icon: BadgePercent, key: "plans" },
  { to: "/checkin", icon: ScanLine, key: "checkin" },
  { to: "/billing", icon: Receipt, key: "billing" },
  { to: "/leads", icon: UserPlus, key: "leads" },
  { to: "/classes", icon: CalendarDays, key: "classes" },
  { to: "/trainers", icon: Dumbbell, key: "trainers" },
  { to: "/reports", icon: BarChart3, key: "reports" },
  { to: "/settings", icon: Settings, key: "settings" },
] as const;

export function Sidebar({ variant = "rail" }: { variant?: "rail" | "drawer" }) {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const pathname = useRouterState({ select: (s) => s.location.pathname });
  const session = useSyncExternalStore(subscribeSession, getSession, getSession);
  const owner = isOwner(session.user);

  function onLogout() {
    void logout();
    void navigate({ to: "/login", replace: true });
  }

  return (
    <aside
      className={cn(
        "flex flex-col bg-white",
        variant === "rail"
          ? "h-full w-[248px] border-r border-line p-5"
          : "w-full rounded-2xl border border-line p-3",
      )}
    >
      <div className={cn("mb-6 flex items-center gap-3", variant === "rail" ? "px-1" : "px-0")}>
        <img src="/logo.png" alt="" className="h-11 w-auto rounded-xl object-contain" />
        <div>
          <div className="text-lg font-bold tracking-tight text-ink">{t("app.name")}</div>
          <div className="text-xs font-medium text-muted">Admin</div>
        </div>
      </div>

      <nav
        className={cn(
          "flex gap-1.5",
          variant === "rail" ? "flex-1 flex-col" : "flex-wrap",
        )}
      >
        {items
          .filter((item) => !("ownerOnly" in item && item.ownerOnly) || owner)
          .map((item) => {
            const isActive = item.to === "/" ? pathname === "/" : pathname.startsWith(item.to);
            return (
              <Link
                key={item.to}
                to={item.to}
                className={cn(
                  "flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-semibold transition",
                  isActive
                    ? "bg-ink text-white shadow-md shadow-ink/20"
                    : "text-ink-soft hover:bg-surface",
                )}
              >
                <item.icon className="size-4.5 opacity-90" />
                {t(`nav.${item.key}`)}
              </Link>
            );
          })}
      </nav>

      <button
        type="button"
        onClick={() => void onLogout()}
        className="mt-4 flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-semibold text-muted transition hover:bg-surface hover:text-ink"
      >
        <LogOut className="size-4.5" />
        {t("nav.logout")}
      </button>
    </aside>
  );
}
